...
class CreditasPipeline:

    # Database setup
    conn = sqlite3.connect('creditas.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `creditas`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM creditas WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO creditas (date, title, link, content)"
